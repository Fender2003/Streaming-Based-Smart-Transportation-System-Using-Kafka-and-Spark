...
def delivery_report(err, msg):
    if err is not None:
        ...
        print(f'Delivery report successful: Delivered to {msg.topic()} [{msg.partition()}]')
...
def simulate_journey(producer, device_id):
    while True:
        ...
        traffic_camera_data = generate_traffic_camera_data(device_id, vehicle_data['timestamp'],vehicle_data['location'],camera_id='Nikon-Cam1')
        ...
        if(vehicle_data['location'][0]>= BIRMINGHAM_COORDINATES['latitude'] and vehicle_data['location'][1] <= BIRMINGHAM_COORDINATES['longitude']):
            print('Vehicle has reached Biringham. Simulation ending...')
            break
